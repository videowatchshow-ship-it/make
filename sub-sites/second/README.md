...
**URL**: https://second.cent-solution.online/  
**서버 경로**: `/var/www/sites/second/`  
**서비스**: `systemd second` (또는 `site-second`)  
...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | `/api/second/accounts` | 계정 목록 (accounts.json) |
| PATCH | `/api/second/accounts/:email/login-issue` | 로그인 문제 등록·수정·해제 |
| GET | `/api/second/login-issues` | 문제 계정 목록 |
