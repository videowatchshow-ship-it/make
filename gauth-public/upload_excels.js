...
function normalizeTotp(s) {
  if (!s) return '';
  let n = String(s).toUpperCase().replace(/[\s\-_=]/g, '');
  n = n.replace(/0/g, 'O').replace(/1/g, 'I').replace(/8/g, 'B');
  n = n.replace(/[^A-Z2-7]/g, '');
  return n || '';
}
...
function isGoogleBackupCode(s) {
  s = String(s || '').trim();
  const parts = s.split(/\s+/);
  if (parts.length >= 4 && parts.length <= 12 && parts.every(p => /^[a-z0-9]{4}$/i.test(p))) return true;
  const valid = [];
  for (const p of parts) { if (/^[a-z0-9]{4}$/i.test(p)) valid.push(p); else break; }
  if (valid.length >= 4) return true;
  return false;
}

function extractGoogleBackupCode(s) {
  s = String(s || '').trim();
  const parts = s.split(/\s+/);
  const valid = [];
  for (const p of parts) { if (/^[a-z0-9]{4}$/i.test(p)) valid.push(p); else break; }
  if (valid.length >= 4) return valid.join(' ');
  return s;
}
...
function isTotpLike(s) {
  ...
  if (isGoogleBackupCode(s)) return true;
...
function analyzeColumns(rows) {
  ...
  for (let c = 0; c < maxCols; c++) {
    const values = [];
    let emails = 0, totps = 0, urls = 0, phones = 0, codes = 0, dates = 0, pureBase32 = 0, nonEmpty = 0;
    for (let r = 0; r < rows.length; r++) {
      ...
      if (stripped.length >= 16 && /^[A-Z2-7]+$/.test(stripped)) pureBase32++;
    }
    stats.push({ col: c, nonEmpty, emails, totps, urls, phones, codes, dates, pureBase32, values });
  }
  ...
  let bestTotp = null, bestTotpScore = 0;
  for (const s of stats) {
    if (used.has(s.col) || s.nonEmpty === 0) continue;
    const ratio = s.totps / s.nonEmpty;
    if (ratio <= 0.03) continue;
    const purity = s.pureBase32 / Math.max(s.totps, 1);
    const score = ratio * (1 + purity * 2);
    if (score > bestTotpScore) {
      bestTotpScore = score;
      bestTotp = s.col;
    }
  }
  ...
  for (const s of stats) {
    ...
    if (s.nonEmpty > 0) {
      ...
      if (allNumbers) {
        const nums = s.values.map(v => parseInt(v));
        const diffs = nums.slice(1).map((n, i) => n - nums[i]);
        const avgDiff = diffs.reduce((a, b) => a + b, 0) / diffs.length;
        const isSequential = s.values.length > 2 && avgDiff > 0 && avgDiff <= 2 &&
          diffs.every(d => d >= 0 && d <= 2);
        if (isSequential) continue;
      }
      if (s.phones > 0 && s.phones / s.nonEmpty > 0.5) continue;
      if (s.codes > 0 && s.codes / s.nonEmpty > 0.5) continue;
      const koreanCount = s.values.filter(v => /[ㄱ-힝]/.test(v)).length;
      if (s.values.length >= 3 && koreanCount / s.values.length > 0.3) continue;
      const uniqueCount = new Set(s.values.map(v => v.toLowerCase().trim())).size;
      if (s.values.length >= 5 && uniqueCount / s.values.length < 0.3) continue;
  ...
  if (rows.length > 500 && typeof global.__debugAnalyze === 'function') {
    global.__debugAnalyze(rows.length, maxCols, stats.map(s => ({
      col: s.col, nonEmpty: s.nonEmpty, emails: s.emails, totps: s.totps,
      urls: s.urls, phones: s.phones, sample: s.values.slice(0, 3)
    })), mapping);
  }
  return mapping;
}
...
function tryLabelValueExtract(rows, maxCols, sourceFile, sourceMtime) {
  ...
  function flushCur() {
    if (cur.email && isEmail(cur.email)) {
      ...
      let _bc = '';
      if (totpVal && isGoogleBackupCode(totpVal)) { _bc = extractGoogleBackupCode(totpVal); totpVal = ''; }
      accounts.push({
        email: cur.email,
        password: pw,
...
function tryStackedExtract(rows, maxCols, sourceFile, sourceMtime) {
  ...
  return accounts.map(a => {
    ...
    let backupCodes = '';
    if (totp && isGoogleBackupCode(totp)) { backupCodes = extractGoogleBackupCode(totp); totp = ''; }
    return {
      email: a.email,
      password: pw,
...
function bruteForceExtract(rows, sourceFile, sourceMtime) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let c = 0; c < row.length; c++) {
      ...
      let backup_codes = '';
      if (totp_secret && isGoogleBackupCode(totp_secret)) { backup_codes = extractGoogleBackupCode(totp_secret); totp_secret = ''; }
      accounts.push({ email: v, password, totp_secret, backup_codes, recovery_email: recovery, youtube_url: youtube, account_date: _dateVal || null, extra: extraValues, source_file: sourceFile || 'unknown', source_mtime: sourceMtime || Date.now() });
    }
  }
  return accounts;
}
...
function extractAccountsFromSheet(sheet, sourceFile, sourceMtime) {
  ...
  for (let i = startRow; i < rows.length; i++) {
    ...
    let youtube = mapping.youtube !== undefined ? String(row[mapping.youtube] || '').trim() : '';
    const dateRaw = mapping.date !== undefined ? String(row[mapping.date] || '').trim() : '';

    if (password && /%[0-9A-Fa-f]{2}/.test(password)) {
      try { password = decodeURIComponent(password); } catch (_) {}
    }
    if (password && !totpRaw && mapping.totp === undefined && isTotpLike(password)) {
      const _pw = password.toUpperCase().replace(/[\s\-_=]/g, '');
      const _b32 = _pw.replace(/[^A-Z2-7]/g, '');
      if (_pw.length > 0 && _b32.length / _pw.length >= 0.9 && _b32.length >= 16) { totpRaw = password; password = ''; }
    }
    ...
    let recoveryBackup = '';
    const _dbg = typeof global.__debugTrackEmail === 'function' && email.toLowerCase().indexOf('mendezjudith754') >= 0;
    if (_dbg) global.__debugTrackEmail('PRE', {email, password, totpRaw, recovery, youtube, recoveryBackup, isEmailRecovery: isEmail(recovery), isGBC: recovery ? isGoogleBackupCode(recovery) : false, isTL: recovery ? isTotpLike(recovery) : false});
    if (recovery && !isEmail(recovery)) {
      if (isGoogleBackupCode(recovery) || isTotpLike(recovery)) { recoveryBackup = recovery; }
      recovery = '';
    }
    if (_dbg) global.__debugTrackEmail('POST_RECOVERY', {recoveryBackup, recovery});
    ...
    let totpFinal = '';
    if (totpRaw) {
      if (isGoogleBackupCode(totpRaw)) { totpFinal = extractGoogleBackupCode(totpRaw); }
      else if (isTotpLike(totpRaw)) { totpFinal = normalizeTotp(totpRaw); }
      else if (isUrlLike(totpRaw)) { if (!youtube) youtube = totpRaw; }
      else if (isEmail(totpRaw) && normalizeEmail(totpRaw) !== normalizeEmail(email)) { if (!recovery) recovery = totpRaw; }
    }
    if (_dbg) global.__debugTrackEmail('POST_TOTP_RAW', {totpRaw, totpFinal, recoveryBackup});
    if (!totpFinal && recoveryBackup) {
      if (isGoogleBackupCode(recoveryBackup)) { totpFinal = extractGoogleBackupCode(recoveryBackup); }
      else if (isTotpLike(recoveryBackup)) { totpFinal = normalizeTotp(recoveryBackup); }
    }
    if (_dbg) global.__debugTrackEmail('POST_RESCUE', {totpFinal});
    if (!totpFinal && youtube && !isUrlLike(youtube)) {
      if (isGoogleBackupCode(youtube)) { totpFinal = extractGoogleBackupCode(youtube); youtube = ''; }
      else if (isTotpLike(youtube)) { totpFinal = normalizeTotp(youtube); youtube = ''; }
    }

    if (!totpFinal && password && mapping.totp === undefined && (isGoogleBackupCode(password) || isTotpLike(password))) {
      if (isGoogleBackupCode(password)) { totpFinal = extractGoogleBackupCode(password); }
      else { totpFinal = normalizeTotp(password); }
      password = '';
    }
    ...
    if (!totpFinal) {
      for (let c = 0; c < row.length; c++) {
        ...
        const cv = String(row[c] || '').trim();
        if (!cv) continue;
        if (isGoogleBackupCode(cv)) { totpFinal = extractGoogleBackupCode(cv); break; }
        if (isTotpLike(cv)) { totpFinal = normalizeTotp(cv); break; }
      }
    }

    if (!password) {
      for (let c = 0; c < row.length; c++) {
        if (c === mapping.email) continue;
        if (usedCols.has(c)) continue;
        const cv = String(row[c] || '').trim();
        if (!cv || cv.length < 4) continue;
        if (isEmail(cv) || isUrlLike(cv) || isPhoneNumber(cv) || isSixDigitCode(cv)) continue;
        if (/^\d+$/.test(cv) || /^[ㄱ-힝\s]+$/.test(cv)) continue;
        if (isGoogleBackupCode(cv) || isTotpLike(cv)) continue;
        password = cv;
        break;
      }
    }
    ...
    if (totpRaw && !totpFinal && !isUrlLike(totpRaw) && !(isEmail(totpRaw) && normalizeEmail(totpRaw) !== normalizeEmail(email))) {
      extra.push(totpRaw);
    }

    let backupCodes = '';
    if (totpFinal && isGoogleBackupCode(totpFinal)) {
      backupCodes = totpFinal;
      totpFinal = '';
    }
...
function recoverMissingFieldsFromExtra(accounts) {
  for (const a of accounts) {
    ...
    for (const raw of a.extra) {
      ...
      if (!v) continue;
      if (!a.backup_codes && isGoogleBackupCode(v)) { a.backup_codes = extractGoogleBackupCode(v); continue; }
      ...
      if (!a.password && !isEmail(v) && !isTotpLike(v) && !isUrlLike(v) && !isSixDigitCode(v) && !isPhoneNumber(v) && !/^\d+$/.test(v) && !/^[ㄱ-힝\s]+$/.test(v)) {
        a.password = v;
        continue;
      }
      remaining.push(v);
...
function parseExcelFile(filePath, originalName) {
  ...
  const allAccounts = [];
  const baseName = (originalName || path.basename(filePath)).normalize('NFC');
  ...
  for (const a of allAccounts) {
    ...
    if (!prev) { byEmail.set(key, a); continue; }
    if (scoreEntry(a) > scoreEntry(prev)) byEmail.set(key, a);
  }
        ...
        for (const fp of files) {
          try {
            ...
            for (const a of result.accounts) {
              a.source_file = path.basename(fp).normalize('NFC');
              if (!a.source_mtime) a.source_mtime = st.mtimeMs;
              if (!a.account_date) {
                const fnDate = parseDateFromFilename(path.basename(fp), new Date(st.mtimeMs).getFullYear());
                if (fnDate) a.account_date = fnDate;
              }
            }
