...
FIX = {
    ...
    ("2026-07-25","16:47"): dict(name="마키아벨리", deposit="1000", type="캐쉬", result="Lose", note="바인 $1,000 (16:43 입금) [이미지확정]"),
...
# 07-25 15:27 은 두 건: 바인 $1,000(기존 유지) + 역송 $3,000 (빈 행에 배정)
for m in ms:
    if (m.get("date"), m.get("start")) == ("2026-07-25","15:27") and not str(m.get("deposit") or "").strip():
        m.update(dict(name="마키아벨리", deposit="3000", type="환전", result="Win", note="역송 $3,000 [이미지확정]"))
        print("  금액확정: 2026-07-25 15:27 마키아벨리 역송 $3,000 (바인 $1,000과 별건)")
